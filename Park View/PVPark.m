...
-(instancetype) initWithFilename:(NSString *)filename
{
    if (self)
    {
        ...
        _boundry = malloc(sizeof(CLLocationCoordinate2D)*_boundaryPointsCount);
        
        for (int i = 0; i < _boundaryPointsCount; i++)
        {
            CGPoint p = CGPointFromString(boundaryPoints[i]);
            _boundry[i] = CLLocationCoordinate2DMake(p.x, p.y);
        }
    }
    return self;
}
...
{
    return CLLocationCoordinate2DMake(self.overlayBottomRightCoordinate.latitude, self.overlayBottomRightCoordinate.longitude);
}
